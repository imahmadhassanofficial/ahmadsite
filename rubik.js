...
function Rubik(element, dimensions, background) {
  ...
  background = background || 0xDAE4DF;
  ...
  camera.position = new THREE.Vector3(-25, 25, 35);
  ...
  var colours = [0xC41E3A, 0x009E60, 0x0051BA, 0xFF5800, 0xFFD500, 0xFFFFFF],
      blackMaterial = new THREE.MeshLambertMaterial({ color: 0x000000, ambient: 0x000000 }),
  ...
  function newCube(x, y, z) {
    ...
    // Create array of materials with all faces black initially
    var materials = [
      blackMaterial, // right
      blackMaterial, // left
      blackMaterial, // top
    ...
    // Only color the exterior faces based on position
    if (Math.abs(x) >= maxExtent - increment/2) {
      materials[x > 0 ? 0 : 1] = faceMaterials[x > 0 ? 0 : 1];
    }
    if (Math.abs(y) >= maxExtent - increment/2) {
      materials[y > 0 ? 2 : 3] = faceMaterials[y > 0 ? 2 : 3];
    }
    if (Math.abs(z) >= maxExtent - increment/2) {
      materials[z > 0 ? 4 : 5] = faceMaterials[z > 0 ? 4 : 5];
    }
